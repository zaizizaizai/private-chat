...
smalltalk.prompt('身份验证', '你的名字?', '').then(function(name){
    ...
    //监听联系人加入
    socket.on('user joined', function(other, index) {
        //设置新加入人的头像
        incomeHtml(other, 'src/img/head.jpg');
        console.log(other + '加入');
        //提示音
        $('#ding')[0].play();
        //浏览器右上角提示新人加入
        showNotice('src/img/head.jpg', other, '上线了');
    });
    ...
    socket.on('receive private message', function(data) {
        ...
        //显示消息
        //TODO:消息长度控制，自动转行
        $('#' + hex_md5(data.addresser) + '.chat-msg')
        .append('<li><img src = "'+head+'"><span class = "speak">'+data.body+'</span></li>');
        //当聊天界面被隐藏时，右上角提示收到消息
        if(document.hidden) {
    ...
    //消息框显示消息
	function incomeHtml(other,head){
		$('#session').append('<li id="li'+hex_md5(other)+'"><img src="'+head+'"><span class="nick-name">'+other+'</span></li>');
		var html = '';
		html+='<div id="'+hex_md5(other)+'" data-n="'+other+'" class="chat"><div class="main">';
		html+='<div class="message"><div class="head"><p>'+other+'</p></div>';
